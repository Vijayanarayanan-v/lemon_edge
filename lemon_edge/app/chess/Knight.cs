...
public class Knight : IChessPiece {
    ...
    int[] rowVector = { 1, 1, -1, -1, 2, 2, -2, -2 };
    int[] columnVector = { 2, -2, -2, 2, 1, -1, -1, 1 };


    public Knight(GridLayout? gridLayout) {
    ...
    public List<CellPosition> GetCellPosition(CellPosition currentPosition) {
        ...
        if(_gridLayout is null)
            throw new LayoutNullException($"Cannot find valid position for {nameof(Knight)},Grid shouldn't be null");
